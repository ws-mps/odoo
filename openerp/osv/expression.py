        ...
        while self.stack:
            ...
            # Applying recursivity on field(one2many)
                ...
                if column._obj != model._name:
                    dom = HIERARCHY_FUNCS[operator](left, ids2, comodel, context=context, prefix=column._obj)
                else:
                    dom = HIERARCHY_FUNCS[operator]('id', ids2, model, context=context, parent=left)
                for dom_leaf in reversed(dom):
