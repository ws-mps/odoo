...
class stock_quant(osv.osv):
    ...
    def _get_quant_from_product(self, cr, uid, ids, context=None):
        return self.pool['stock.quant'].search(cr, uid, [('product_id', 'in', ids)], context=context)
    ...
    _columns = {
        ...
        'inventory_value': fields.function(_calc_inventory_value, string="Inventory Value", type='float', readonly=True,
                                           store={'stock.quant': (lambda self, cr, uid, ids, ctx: ids, ['qty'], 10),
                                                  'product.product': (_get_quant_from_product, ['standard_price'], 10)}),
